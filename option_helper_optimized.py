...
def build_transition_sequence(df: pd.DataFrame) -> list:
    ...
    for i in range(1, n):
        ...
        transitions.append({
            ...
            "pct":        pct,
            "date":       df.index[i],
            "row_idx":    i,   # position within df — lets walk-forward loops take
                               # cheap prefixes instead of rescanning df each time
        })
    return transitions


# ─────────────────────────────────────────────
# TRANSITION CACHE  (perf: avoid rescanning df at every walk-forward step)
# ─────────────────────────────────────────────
# build_transition_sequence(df.iloc[:i+1]) is safe to call once against the
# FULL df and then take a prefix, because every transition only looks at two
# adjacent rows — nothing about it depends on rows after position i. Walk-
# forward loops (the threshold-search evaluator and the calibration
# backtest) used to call build_transition_sequence(df_hist) fresh at every
# one of hundreds of stride points, each one rescanning the whole dataframe
# up to that point — O(n) work repeated O(n/stride) times. Building the
# full sequence ONCE and slicing a prefix turns that into O(n) total.
def build_full_transition_cache(df: pd.DataFrame) -> dict:
    transitions = build_transition_sequence(df)
    return {"transitions": transitions, "n": len(transitions)}


def transitions_upto(cache: dict, cutoff_row_idx: int, ptr_state: dict) -> list:
    """
    Returns the FULL prefix of cache['transitions'] with row_idx <=
    cutoff_row_idx (i.e. what build_transition_sequence(df.iloc[:cutoff_row_idx+1])
    would have returned). ptr_state (a plain dict, e.g. {}) is mutated in
    place to remember how far the scan got, so repeated calls with a
    non-decreasing cutoff_row_idx (true of every walk-forward loop in this
    file) resume instead of re-scanning from the start — O(1) amortized
    per call instead of O(n).
    """
    transitions = cache["transitions"]
    ptr = ptr_state.get("ptr", 0)
    n = cache["n"]
    while ptr < n and transitions[ptr]["row_idx"] <= cutoff_row_idx:
        ptr += 1
    ptr_state["ptr"] = ptr
    return transitions[:ptr]


def advance_transitions(cache: dict, cutoff_row_idx: int, ptr_state: dict) -> list:
    """
    Like transitions_upto(), but returns only the NEW transitions since the
    last call (instead of the whole prefix) — what IncrementalStateIndex
    below wants to .extend() with. Requires a non-decreasing cutoff_row_idx
    sequence, same as transitions_upto().
    """
    transitions = cache["transitions"]
    ptr = ptr_state.get("ptr", 0)
    start = ptr
...
# ─────────────────────────────────────────────
# INCREMENTAL STATE INDEX  (perf: cache the state map across walk-forward steps)
# ─────────────────────────────────────────────
class IncrementalStateIndex:
    """
    Produces the exact same output as build_state_index(transitions,
    thresholds_by_type), but incrementally: extend() only touches
    transitions that are NEW since the last call, instead of rebuilding
    state_map/unconditional_by_type from scratch every time.

    This is only valid when thresholds_by_type stays FIXED for the life of
    one instance — which is true everywhere it's used below (evaluate_
    thresholds_walkforward evaluates one fixed candidate threshold set
    across a growing window of history). Because a transition's regime
    only depends on its own pct and its own type's thresholds, once it's
    classified it never needs to be revisited even as more data arrives.

    Previously, a walk-forward loop with S stride points over N total
    transitions rebuilt the full state map at every point: O(S x N) work.
    With this class it's O(N) total — each transition is processed once.

    snapshot() additionally only re-runs np.array() on dict entries that
    changed since the last snapshot (tracked via a dirty-key set), rather
    than converting the whole state map every call.
    """

    def __init__(self, thresholds_by_type: dict):
        self.thresholds_by_type = thresholds_by_type
        self.transitions = []
        self.regimes = []
        self.state_map = defaultdict(list)
        self.unconditional_by_type = defaultdict(list)
        self._last_state_i = 0   # state_map fully built for local index 1..this (inclusive)
        self._dirty_state_keys = set()
        self._dirty_uncond_keys = set()
        self._state_array_cache = {}
        self._uncond_array_cache = {}

    def extend(self, new_transitions: list):
        if not new_transitions:
            return
        for t in new_transitions:
            self.transitions.append(t)
            self.regimes.append(classify_regime(t["pct"], *self.thresholds_by_type[t["type_idx"]]))
            self.unconditional_by_type[t["type_idx"]].append(t["pct"])
            self._dirty_uncond_keys.add(t["type_idx"])

        n = len(self.transitions)
        end_i = n - 2   # mirrors build_state_index's range(1, len(transitions) - 1)
        for i in range(self._last_state_i + 1, end_i + 1):
            state_tuple = (self.regimes[i - 1], self.regimes[i])
            next_t      = self.transitions[i + 1]
            key         = (state_tuple, next_t["type_idx"])
            self.state_map[key].append(next_t["pct"])
            self._dirty_state_keys.add(key)
        if end_i > self._last_state_i:
            self._last_state_i = end_i

    def snapshot(self) -> tuple:
        """Returns (state_map, unconditional_by_type, regimes, current_state, current_next_type_idx) — same shape as build_state_index()."""
        for key in self._dirty_state_keys:
            self._state_array_cache[key] = np.array(self.state_map[key])
        self._dirty_state_keys.clear()

        for type_idx in self._dirty_uncond_keys:
            self._uncond_array_cache[type_idx] = np.array(self.unconditional_by_type[type_idx])
        self._dirty_uncond_keys.clear()

        current_state         = (self.regimes[-2], self.regimes[-1])
        current_next_type_idx = (self.transitions[-1]["type_idx"] + 1) % N_TYPES
        return self._state_array_cache, self._uncond_array_cache, self.regimes, current_state, current_next_type_idx
...
def evaluate_thresholds_walkforward(
    ...
    thresholds_by_type: dict,
    config: dict = None,
    transitions_cache: dict = None,
) -> dict | None:
    """
    transitions_cache: optional, from build_full_transition_cache(df).
    Callers that invoke this many times against the SAME df (e.g. the
    coordinate-ascent grid search below, which calls this once per
    candidate) should build the cache once and pass it in — it turns what
    used to be a full df rescan at every one of hundreds of walk-forward
    points into a single O(n) pass. If omitted, a cache is built locally
    (still correct, just no reuse across calls).
    """
    ...
    if transitions_cache is None:
        transitions_cache = build_full_transition_cache(df)
    ptr_state = {}
    builder   = IncrementalStateIndex(thresholds_by_type)
    ...
    for i in range(min_hist, last_valid + 1, stride):
        builder.extend(advance_transitions(transitions_cache, i, ptr_state))
        if len(builder.transitions) < 50:
            continue
...
def optimize_thresholds_full(df: pd.DataFrame, config: dict = None, verbose: bool = True) -> tuple:
    ...
    # Built once, reused by every evaluate_thresholds_walkforward() call
    # below (hundreds/thousands of them) instead of each one rescanning df
    # from scratch — see build_full_transition_cache()'s docstring.
    transitions_cache = build_full_transition_cache(df.sort_index())
        ...
        for type_idx in range(N_TYPES):
            ...
            for lo in config["bear_grid"]:
                for hi in config["bull_grid"]:
                    ...
                    metrics = evaluate_thresholds_walkforward(df, candidate, config, transitions_cache=transitions_cache)
                    if metrics is None:
                        continue
                    if best is None or metrics["overall_score"] > best["overall_score"]:
...
def evaluate_calibration_for_ticker(
    ...
    # Built once, sliced per step below instead of rescanning df at every
    # stride point (see build_full_transition_cache()'s docstring). Note:
    # thresholds are re-optimized from truncated history at each step here,
    # so build_state_index() itself still has to rebuild (regimes depend on
    # thresholds) — IncrementalStateIndex isn't applicable in this loop —
    # but the transitions retrieval that used to dominate this cost is now
    # O(1) amortized instead of O(n) per step.
    transitions_cache = build_full_transition_cache(df)
    ptr_state = {}

    for i in eval_indices:
        transitions = transitions_upto(transitions_cache, i, ptr_state)
